    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Login(User model)
        {
            ...
            if (CheckLoginEmail(model))
            {
                ...
                {
                    TempData["LoginError"] = "Email ork password is incorrect";
                    return RedirectToAction("Index");
                }
            }
            else
        ...
        public async Task<IActionResult> CommentFun(CommentViewModel vm)
        {
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("Edit", new { id = @HttpContext.Session.GetInt32("_UserID") });
            await dbContext.SaveChangesAsync();

            return RedirectToAction("personal", new {id=@HttpContext.Session.GetInt32("_UserID") });
